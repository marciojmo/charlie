...
intent_classifier = intents.IntentClassifier()
        ...
        try:
            with rabbitmq.get_rabbitmq_connection() as connection:
                ...
                channel.basic_consume(
                    queue=settings.RABBITMQ_INCOMING_MESSAGES_QUEUE,
...
def process_message(channel, method, properties, body) -> None:
    ...
    try:
        ...
        message = models.IncomingMessage.model_validate_json(body.decode("utf-8"))
        if not message.is_group:
            intent = intent_classifier.get_label(message.text)
            model_answer = chit_chat.get_chit_chat_answer(intent["intent"])
            outgoing_message = models.OutgoingMessage(
                number=message.number,
                text=model_answer.format(message.username),
            )
            rabbitmq.publish_message(
                settings.RABBITMQ_OUTGOING_MESSAGES_QUEUE, outgoing_message
            )
